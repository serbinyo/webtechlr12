    ...
    function loadFromFile()
    {
        ...
        if (is_file($source)) {
            if ($_FILES["messages"]["name"] === 'messages.inc') {
                if (copy($source, $dest)) {
                    return $message = '<h3 style="text-align: center; color: blue;">Файл успешно загружен</h3>';
                } else {
                    return $message = '<h3 style="text-align: center; color: red;">Ошибка загрузки файла</h3>';
                }
            } else {
                return $message = '<h3 style="text-align: center; color: red;">Ошибка. Требуется messages.inc</h3>';
            }
        } else {
            return $message = '<h3 style="text-align: center; color: red;">Ошибка чтения файла</h3>';
        }
    }
}
